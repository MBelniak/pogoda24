...
export default class Traffic extends React.Component<{}, TrafficState> {
    ...
    private async fetchGatheredData() {
        try {
            const response = await fetchApi('api/views/gathered', {
    ...
    private async fetchSiteViews() {
        this.setState({siteViewsChartData: undefined});
        try {
            const response = await fetchApi('api/views/site?daysBack=' + this.siteViewsDaysBack, {
                signal: this.abortController.signal
            });
            if (response && response.ok) {
                const siteViewsData: TrafficDTO[] = await response.json();
                this.setState({
